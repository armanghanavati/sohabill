...
interface taxState {
  ...
  buyerPersonFilterField: BuyerPersonFilterField;
}
interface MyData { }
...
const initialState: taxState = {
  billItem: {},
  printBill: {},
  allBillList: { hasNextPage: false, hasPreviousPage: false, items: [], pageNumber: 0, pageSize: 0, totalCount: 0, totalPages: 0 },
  buyerPersonFilterField: {},
  pattern: [],
...
export const handleBuyerPersonList = createAsyncThunk(
  ...
  async (data: BuyerPersonFilterField, { dispatch, getState }) => {
    const postData: BuyerPersonFilterField = {
      personCode: data?.personCode || "",
      economicCode: data?.economicCode || "",
      postCode: data?.postCode || "",
      buyerBranchCode: data?.buyerBranchCode || "",
...
export const handleGetBill = createAsyncThunk(
  ...
  async (billId: number, { dispatch, getState }: any) => {
    ...
    try {
      ...
      dispatch(RsetShowLoading({ value: false }))
      dispatch(RsetAllFieldSteps({ rowTableBuyerPer: resGetBill?.data?.result?.bill?.buyerPersonId, editBillItem: resGetBill?.data?.result?.bill }))
...
export const handleGetFields: ReduxToolkitType = createAsyncThunk(
  ...
  async (obj: ServiceGetField, { dispatch, getState }: any) => {
    try {
      ...
      const resGetFields = await getFields(obj?.patternId || patternTypeId?.billPattern?.id, obj?.typeId || patternTypeId?.billType?.id, obj?.subjectId);
...
export const handlePrintBill = createAsyncThunk(
  "tax/handlePrintBill",
  async (billId: number, { dispatch, getState }: any) => {
    dispatch(RsetShowLoading({ value: true, btnName: billId }))
    try {
...
export const postSaveAndUpsertData = (obj: any,) => {
  const allCountList = (obj?.productList)?.map((item: any, index: number) => {
    ...
    return {
      ...
      currencyCode: (item?.currencyCode?.id ?? item?.currencyCode?.abbreviation) || null,
      priceBeforeDiscount: StringHelpers.operationRemoveSep(item?.priceBeforeDiscount) || null,
      discount: StringHelpers.operationRemoveSep(item?.discount) || null,
      priceAfterDiscount: StringHelpers.operationRemoveSep(item?.priceAfterDiscount) || null,
      equivalentWithRial: StringHelpers.operationRemoveSep(item?.equivalentWithRial) || null,
      valueIncreasedTaxRate: item?.valueIncreasedTaxRate === "" ? null : Number(item?.valueIncreasedTaxRate),
      itemValueIncreasedTaxPrice: StringHelpers.operationRemoveSep(item?.itemValueIncreasedTaxPrice) || null,
      otherTaxRate: StringHelpers.operationRemoveSep(item?.otherTaxRate) || null,
      otherTaxPrice: StringHelpers.operationRemoveSep(item?.otherTaxPrice) || null,
      otherTaxSubject: (item?.otherTaxSubject) || null,
      otherLegalFundsRate: (item?.otherLegalFundsRate) || null,
      otherLegalFundsPrice: StringHelpers.operationRemoveSep(item?.otherLegalFundsPrice) || null,
      otherLegalFundsSubject: (item?.otherLegalFundsSubject) || null,
      registerContractId: (item?.registerContractId) || null,
      constructionFee: StringHelpers.operationRemoveSep(item?.constructionFee) || null,
      sellerProfit: StringHelpers.operationRemoveSep(item?.sellerProfit) || null,
      commission: StringHelpers.operationRemoveSep(item?.commission) || null,
      totalItemsPrice: StringHelpers.operationRemoveSep(item?.totalItemsPrice) || null,
    };
  ...
  const allCountPay = obj?.listPay?.map((item: any, index: number) => {
    return {
      paymentDate: typeof item?.paymentDate === "object" ? StringHelpers.convertDatePer(item?.paymentDate) : item?.paymentDate || null,
  ...
  const postData: Checkout = {
    inputModel: {
      bill: {
        id: 0,
        issueDate: typeof obj?.propsData?.step1_2?.issueDate === "object" ? StringHelpers.convertDatePer(obj?.propsData?.step1_2?.issueDate) : obj?.propsData?.step1_2?.issueDate || null,
        ...
        customsLicenseNumber: obj?.propsData?.step1_2?.customsLicenseNumber || null,
        customsCode: obj?.propsData?.step1_2?.customsCode || null,
        cotageNumber: obj?.propsData?.step1_2?.cotageNumber || null,
        cotageDate: typeof obj?.propsData?.step1_2?.cotageDate === "object" ? StringHelpers.convertDatePer(obj?.propsData?.step1_2?.cotageDate) : obj?.propsData?.step1_2?.cotageDate || null,
        ...
        creditPrice: StringHelpers.operationRemoveSep(obj?.propsData?.step1_2?.creditPrice) || null,
        ...
        article17TaxPrice: Number(StringHelpers.operationRemoveSep(obj?.propsData?.step1_2?.article17TaxPrice)) || null,
...
const taxSlice = createSlice({
  ...
  reducers: {
    RsetBuyerPersonFilterField: (
      state,
      action: PayloadAction<BuyerPersonFilterField>
    ) => {
  ...
  extraReducers: (builder) => {
    builder
      .addCase(handleGetInvoiceTemp.fulfilled, (state, action: PayloadAction<any>) => {
        try {
      ...
      .addCase(handleGetBillsList.fulfilled, (state, action: PayloadAction<AllServiceMaster>) => {
        try {
          if (action.payload.code === 0) {
      ...
      // })
      .addCase(handlePrintBill.fulfilled, (state, action: PayloadAction<any>) => {
