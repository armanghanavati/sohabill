...
const BatchEntryPage: React.FC<Props> = ({ billsErrors, setBillsErrors }) => {
  ...
  const getColRows = (sheets: WorkSheet[]) => {
    ...
    billsHeader = utils.sheet_to_json<string[]>(sheets[0], {
      header: 1,
      range: billsRange,
    })[0];
  ...
  return (
    <>
      ...
      <section className="tw-mx-auto tw-w-full tw-justify-start">
        ...
        {!!file && (
          <div className="d-flex px-4 flex-row align-items-center">
            ...
            </Col>
            {
              ...
              true ? (
                <Col className="w-auto" md={4}>
                  <Button
                    title="دانلود فایل به همراه خطاها"
                    onClick={exportXLSX}
                  />
                </Col>
              ) : excelBase64 !== "" ? (
